...
export default function DashboardTalleristaAC() {
  ...
  function agregarItem(m: any) {
    const yaEsta = itemsSolicitud.find(x => x.id === m.id)
    if (yaEsta) return
    ...
    <main className="min-h-screen bg-[#F0FAFB]">
      <div className="bg-[#0F3A42] px-4 py-3">
        <div className="flex justify-between items-center">
          <div>
            <div className="text-white font-bold text-lg tracking-wide">Tallerista AC</div>
            <div className="text-[#7ADCE8] text-xs mt-0.5">{perfil.nombre} · Turno {perfil.turno}</div>
          </div>
          <div className="bg-[#0A2830] text-[#7ADCE8] text-xs font-bold px-3 py-1 rounded-full tracking-wide uppercase">TALL·AC</div>
        </div>
      </div>

      {/* PANEL STOCK SOLICITUD */}
      {showSolicitud && (
              ...
              <div className="mt-3 border-t border-[#E8F4F7] pt-3">
                {itemsSolicitud.map(m => (
                  <div key={m.id} className="flex items-center gap-2 mb-2">
                    ...
                    <input
                      type="number" min={1} value={m.cantidad}
                ...
                <button
                  ...
                  disabled={loadingSolicitud || !ordenActiva}
            ...
            <div className="overflow-y-auto flex-1">
              ...
              {showCierre && (
                <div className="mb-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Detallá lo realizado..."
                    value={obscierre}
              ...
              <div className="flex gap-2 mt-2">
                {ordenDetalle.estado === 'pendiente' && (
                  <button onClick={() => iniciarOrden(ordenDetalle.id)} className="flex-1 bg-[#1ABBD6] text-white font-bold text-sm py-3 rounded-xl">
                    INICIAR
                  </button>
                )}
                {ordenDetalle.estado === 'en_curso' && !showCierre && (
                  <>
                    <button onClick={abrirStock} className="flex-1 bg-[#F0FAFB] border border-[#B2E0E8] text-[#0F3A42] font-bold text-sm py-3 rounded-xl">
                      + INSUMOS
                    </button>
                    <button onClick={() => setShowCierre(true)} className="flex-1 bg-[#3B6D11] text-white font-bold text-sm py-3 rounded-xl">
                      CERRAR
                    </button>
                  </>
                )}
        ...
        {nomenclaturas.length > 1 && (
          <div className="mb-3">
            <select
              ...
              onChange={e => setFiltroNomenclatura(e.target.value)}
            >
