...
export default function DashboardSupervisorAC() {
  ...
  async function cargarDatos(turno: string) {
    const { data: ords } = await supabase.from('ordenes_trabajo')
      ...
      .eq('sector', 'ac').order('created_at', { ascending: false })
    setOrdenes(ords || [])
    const { data: tecs } = await supabase.rpc('get_tecnicos_activos', { p_sector: 'ac', p_turno: turno })
    setTecnicos(tecs || [])
  }
  ...
  async function cargarOrdenesRebotadas() {
    const { data } = await supabase.from('ordenes_trabajo')
      .select('id, numero_orden, titulo, observacion_panol, rebotada_at, sector')
      .eq('estado', 'rebotada').eq('sector', 'ac')
  ...
  async function cargarCheckinsVehiculos() {
    const { data } = await supabase.from('checkins_vehiculos')
      ...
      .eq('sector', 'ac').order('created_at', { ascending: false }).limit(60)
  ...
  async function abrirForm() {
    if (nomenclaturas.length === 0) {
      const { data } = await supabase.from('nomenclaturas').select('*').eq('sector', 'ac').order('codigo', { ascending: true })
  ...
  async function crearOrden() {
    ...
    const { data: nuevaOrden, error } = await supabase.from('ordenes_trabajo').insert({
      titulo: form.titulo, descripcion: form.descripcion, sector: 'ac', estado: 'pendiente',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: C.sub, marginTop: 1 }}>Supervisor AC · Turno {perfil.turno}</div>
            </div>
          </div>
