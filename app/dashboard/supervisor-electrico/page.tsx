...
export default function DashboardSupervisorElectrico() {
  ...
  async function crearOrden() {
    ...
    if (!error && nuevaOrden) {
      // técnicos
      const filasTecnicos = tecnicosSeleccionados.map(tid => ({
        orden_id: nuevaOrden.id,
        tecnico_id: tid,
        cerro: false
      }))
      await supabase.from('orden_tecnicos').insert(filasTecnicos)

      // materiales
      for (const m of materialesOrden) {
        if (m.stock >= m.cantidad) {
          // hay stock → orden_materiales
